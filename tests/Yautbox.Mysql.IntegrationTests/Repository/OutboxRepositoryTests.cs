...
namespace Yautbox.Mysql.IntegrationTests.Repository;
...
public class OutboxRepositoryTests : IAsyncLifetime
{
    ...
    private IMysqlOutboxRepository Create()
    {
        var optionsSnapshot = Substitute.For<IOptionsSnapshot<MysqlOutboxRepositoryOptions>>();

        optionsSnapshot
            .Value
            .Returns(new MysqlOutboxRepositoryOptions { SchemaName = "outbox" });

        return new MysqlOutboxRepository(
            logger: NullLogger<MysqlOutboxRepository>.Instance,
